...
class TowerOfHanoiVisualizer:
    ...
    def calculate_moves(self, num_discs, num_towers):
        ...
        # Solve and record each move
        self.solve_hanoi(0, num_towers-1, num_discs, num_towers)
    ...
    def solve_hanoi(self, begin, end, n, t):
        def hanoi(begin, end, n, t):
            # Record intermediate states
            count = n//(t-2)
            for i in range(2, t-1):
                hanoi3(self.towers[0], self.towers[t-i], self.towers[1], count)
                
            hanoi3(self.towers[0], self.towers[t-1], self.towers[1], n-(count*(t-3)))
            
            for i in range(t-2, 1, -1):
                hanoi3(self.towers[t-i], self.towers[t-1], self.towers[1], count)
                
        def hanoi3(begin, end, temp, n):
            if n == 1:
                end.push(begin.pop())
                # Find the indices of the towers
                begin_idx = self.towers.index(begin)
                end_idx = self.towers.index(end)
                self.record_state(f"Move disc from Tower {begin_idx+1} to Tower {end_idx+1}")
            else:
